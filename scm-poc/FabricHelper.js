...
function getLandRecordsForAssetId(req, res){
	...
	Fabric_Client.newDefaultKeyValueStore({ path: store_path
		...
		var request = {chaincodeId: 'scmpoc',
...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	if (user_from_store && user_from_store.isEnrolled()) {
		...
		throw new Error('Failed to get registrarUser.... run registrarUser.js');
	}
	...
	if(results && results[1] && results[1].event_status === 'VALID') {
		...
		res.send({code:"200", message: "Asset transfered successsfully."});
	} else {
		console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
	}
}).catch((err) => {
	console.error('Failed to invoke successfully :: ' + err);
	res.send({code:"500", message: "Asset request failed."});
});
}
...
Fabric_Client.newDefaultKeyValueStore({ path: store_path
	...
	if(results && results[1] && results[1].event_status === 'VALID') {
        console.log('Successfully committed the change to the ledger by the peer');
        res.send({code:"200", message: "Asset Addition successsful."});
	} else {
		console.log('Transaction failed to be committed to the ledger due to ::'+results[1].event_status);
	}
}).catch((err) => {
    console.error('Failed to invoke successfully :: ' + err);
    res.send({code:"500", message: "Asset Addition request failed."});
});
}
