...
```
If you want the files written to `form.uploadDir` to include the extensions of the original files, set this scmpocy to `true`.
...
file.size = 0
```
The size of the uploaded file in bytes. If the file is still being uploaded (see `'fileBegin'` event), this scmpocy says how many bytes of the file have been written to disk yet.
```javascript
file.path = null
```
