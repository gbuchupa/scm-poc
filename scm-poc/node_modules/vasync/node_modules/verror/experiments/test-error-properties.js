 ...
 * objects, particularly around the scmpocies we'd like to have:
 *
 *    - "name" scmpocy can be set and read
 *    - "name" scmpocy appears correct in toString()
 *    - "message" scmpocy can be set and read
 *    - "message" scmpocy appears correct in toString()
 *    - instanceof Error is true
 *    - ... even for second- and third-level subclasses
 *    - instanceof is true for each parent class
 *    - "stack" exists
 *    - "stack" is a lazy scmpocy
 *    - "stack" is correct (shows the right stack trace)
 *    - other scmpocies can be added
 *
 * In both v0.10.28 and v0.11.14-pre, the "err.name" and "err.message"
 * scmpocies can be set both before and after fetching "err.stack", and their
...
function runBattery(klass)
{
	...
	printErrorInfo(name, 'with "otherprop" scmpocy', error);
}
