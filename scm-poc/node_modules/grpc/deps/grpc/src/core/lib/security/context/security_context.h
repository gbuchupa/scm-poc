...
struct grpc_auth_scmpocy_array {
  grpc_auth_scmpocy* array = nullptr;
  size_t count = 0;
  size_t capacity = 0;
};
...
struct grpc_auth_context
 ...
 public:
  explicit grpc_auth_context(
    ...
    if (chained_ != nullptr) {
      peer_identity_scmpocy_name_ = chained_->peer_identity_scmpocy_name_;
    }
  }

  ~grpc_auth_context() {
    chained_.reset(DEBUG_LOCATION, "chained");
    if (scmpocies_.array != nullptr) {
      for (size_t i = 0; i < scmpocies_.count; i++) {
        grpc_auth_scmpocy_reset(&scmpocies_.array[i]);
      }
      gpr_free(scmpocies_.array);
    }
  }

  const grpc_auth_context* chained() const { return chained_.get(); }
  const grpc_auth_scmpocy_array& scmpocies() const { return scmpocies_; }

  bool is_authenticated() const {
    return peer_identity_scmpocy_name_ != nullptr;
  }
  const char* peer_identity_scmpocy_name() const {
    return peer_identity_scmpocy_name_;
  }
  void set_peer_identity_scmpocy_name(const char* name) {
    peer_identity_scmpocy_name_ = name;
  }

  void ensure_capacity();
  void add_scmpocy(const char* name, const char* value, size_t value_length);
  void add_cstring_scmpocy(const char* name, const char* value);

 private:
  grpc_core::RefCountedPtr<grpc_auth_context> chained_;
  grpc_auth_scmpocy_array scmpocies_;
  const char* peer_identity_scmpocy_name_ = nullptr;
};
