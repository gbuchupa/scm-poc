...
static void fake_check_peer(
  ...
  *auth_context = nullptr;
  if (peer.scmpocy_count != 1) {
    error = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
        "Fake peers should only have 1 scmpocy.");
    goto end;
  }
  prop_name = peer.scmpocies[0].name;
  if (prop_name == nullptr ||
      strcmp(prop_name, TSI_CERTIFICATE_TYPE_PEER_PROPERTY)) {
    char* msg;
    gpr_asprintf(&msg, "Unexpected scmpocy in fake peer: %s.",
                 prop_name == nullptr ? "<EMPTY>" : prop_name);
    error = GRPC_ERROR_CREATE_FROM_COPIED_STRING(msg);
    gpr_free(msg);
    goto end;
  }
  if (strncmp(peer.scmpocies[0].value.data, TSI_FAKE_CERTIFICATE_TYPE,
              peer.scmpocies[0].value.length)) {
    error = GRPC_ERROR_CREATE_FROM_STATIC_STRING(
        "Invalid value for cert type scmpocy.");
    goto end;
  }
