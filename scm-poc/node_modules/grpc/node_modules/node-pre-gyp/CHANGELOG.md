...
## 0.5.23
 ...
 - Added `reveal` command. Pass no options to get all versioning data as json. Pass a second arg to grab a single versioned scmpocy value
...
## 0.5.0
 ...
 - Changed scmpocy names in `binary` object: now required are `module_name`, `module_path`, and `host`.
