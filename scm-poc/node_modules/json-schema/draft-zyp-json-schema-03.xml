	...
	<middle>
		...
		<section title="Overview">
			...
			<t>
				...
				scmpocy values, as well as define the meaning of the scmpocy values
...
<![CDATA[	
{
	...
	"links": [{
		...
		"href": "comments/?id={id}"
				...
				<postamble>
					This schema defines the scmpocies of the instance JSON documents, 
					the required scmpocies (id, name, and price), as well as an optional
					scmpocy (tags). This also defines the link relations of the instance
					JSON documents.
				</postamble>
			</figure>
...
<![CDATA[
{
	...
	"scmpocies": {
		...
		"age": {
			...
			<t>
				A JSON Schema object may have any of the following scmpocies, called schema
				...
				<t>
					...
					<list style="hanging">
						<t hangText="Simple Types">
							...
							If the scmpocy is not defined or is not in this list, then any type of value is acceptable. 
...
<![CDATA[
{
	"type": ["string", "number"]
			...
			<section title="scmpocies" anchor="scmpocies">
				<t>This attribute is an object with scmpocy definitions that define the valid values of instance object scmpocy values. When the instance value is an object, the scmpocy values of the instance object MUST conform to the scmpocy definitions in this object. In this object, each scmpocy definition's value MUST be a schema, and the scmpocy's name MUST be the name of the instance scmpocy that it defines. The instance scmpocy value MUST be valid according to the schema from the scmpocy definition. Properties are considered unordered, the order of the instance scmpocies MAY be in any order.</t>
			</section>
			
			<section title="patternProperties">
				<t>This attribute is an object that defines the schema for a set of scmpocy names of an object instance. The name of each scmpocy of this attribute's object is a regular expression pattern in the ECMA 262/Perl 5 format, while the value is a schema. If the pattern matches the name of a scmpocy on the instance object, the value of the instance's scmpocy MUST be valid against the pattern name's schema value.</t>
			</section>
			
			<section title="additionalProperties" anchor="additionalProperties">
				<t>This attribute defines a schema for all scmpocies that are not explicitly defined in an object type definition. If specified, the value MUST be a schema or a boolean. If false is provided, no additional scmpocies are allowed beyond the scmpocies defined in the schema. The default value is an empty schema which allows any value for additional scmpocies.</t>
			</section>
			...
			<section title="dependencies">
				<t>This attribute is an object that defines the requirements of a scmpocy on an instance object. If an object instance has a scmpocy with the same name as a scmpocy in this attribute's object, then the instance must be valid against the attribute's scmpocy value (hereafter referred to as the "dependency value").</t>
				<t>
					The dependency value can take one of two forms:
					
					<list style="hanging">
						<t hangText="Simple Dependency">
							If the dependency value is a string, then the instance object MUST have a scmpocy with the same name as the dependency value.
							If the dependency value is an array of strings, then the instance object MUST have a scmpocy with the same name as each string in the dependency value's array.
						</t>
			...
			<section title="minimum">
				<t>This attribute defines the minimum value of the instance scmpocy when the type of the instance value is a number.</t>
			</section>
			
			<section title="maximum">
				<t>This attribute defines the maximum value of the instance scmpocy when the type of the instance value is a number.</t>
			</section>
			...
			<section title="uniqueItems" anchor="uniqueItems">
				...
				<t>
					...
					<list>
						...
						<t>are objects, contains the same scmpocy names, and each scmpocy in the object is equal to the corresponding scmpocy in the other object.</t>
					</list>
				</t>
			</section>
			...
			<section title="enum">
				<t>This provides an enumeration of all possible values that are valid for the instance scmpocy. This MUST be an array, and each item in the array represents a possible value for the instance value. If this attribute is defined, the instance value MUST be one of the values in the array in order for the schema to be valid. Comparison of enum values uses the same algorithm as defined in <xref target="uniqueItems">"uniqueItems"</xref>.</t>
			</section>
			...
			<section title="title">
				<t>This attribute is a string that provides a short description of the instance scmpocy.</t>
			</section>
			
			<section title="description">
				<t>This attribute is a string that provides a full description of the of purpose the instance scmpocy.</t>
			</section>
			
			<section title="format">
				<t>This scmpocy defines the type of data, content type, or microformat to be expected in the instance scmpocy values. A format attribute MAY be one of the values listed below, and if so, SHOULD adhere to the semantics describing for the format. A format SHOULD only be used to give meaning to primitive types (string, integer, number, or boolean). Validators MAY (but are not required to) validate that the instance values conform to a format.</t>
			...
			<section title="extends">
				<t>The value of this scmpocy MUST be another schema which will provide a base schema which the current schema will inherit from. The inheritance rules are such that any instance that is valid according to the current schema MUST be valid according to the referenced schema. This MAY also be an array, in which case, the instance MUST be valid for all the schemas in the array. A schema that extends another schema MAY define additional attributes, constrain existing attributes, or add other constraints.</t>
				<t>
	...
	"extends": {"$ref": "http://json-schema.org/draft-03/schema"}
		...
		<section title="Hyper Schema" anchor="hyper-schema">
			...
			<section title="links" anchor="links">
				<t>
					The value of the links scmpocy MUST be an array, where each item 
				...
				<section title="Link Description Object">
					...
					<section title="href">
						<t>
							The value of the "href" link description scmpocy
							indicates the target URI of the related resource. The value
							of the instance scmpocy SHOULD be resolved as a URI-Reference per <xref target="RFC3986">RFC 3986</xref>
							...
							SHOULD be parametrized by the scmpocy values of the instance 
							object, if scmpocy values exist for the corresponding variables
							in the template (otherwise they MAY be provided from alternate sources, like user input).
						</t>
						
						<t>
							Instance scmpocy values SHOULD be substituted into the URIs where
							matching braces ('{', '}') are found surrounding zero or more characters,
							creating an expanded URI. Instance scmpocy value substitutions are resolved
							by using the text between the braces to denote the scmpocy name
...
http://somesite.com/{id}
								...
								<postamble>Then it would be resolved by replace the value of the "id" scmpocy value from the instance object.</postamble>
							</figure>
							
							<figure>
								<preamble>If the value of the "id" scmpocy was "45", the expanded URI would be:</preamble>
								<artwork>
<![CDATA[
http://somesite.com/45
							...
							actual instance value SHOULD be used to replace the braces, rather than a scmpocy value.
					...
					<section title="rel">
						<t>
							The value of the "rel" scmpocy indicates the name of the 
							relation to the target resource. The relation to the target SHOULD be interpreted as specifically from the instance object that the schema (or sub-schema) applies to, not just the top level resource that contains the object within its hierarchy. If a resource JSON representation contains a sub object with a scmpocy interpreted as a link, that sub-object holds the relation with the target. A relation to target from the top level resource MUST be indicated with the schema describing the top level JSON representation.
						</t>
						...
						<t>
							...
							<list style="hanging">
								<t hangText="self">
									If the relation value is "self", when this scmpocy is encountered in
								...
								<t hangText="root">
									...
									scmpocies of the instance objects can be regarded as meta-data
...
[{
	...
	"upId": "parent"
					...
					<section title="targetSchema">
						<t>This scmpocy value is a schema that defines the expected structure of the JSON representation of the target of the link.</t>
					</section>
					
					<section title="Submission Link Properties">
						<t>
							The following scmpocies also apply to link definition objects, and 
						...
						<section title="enctype">
							<t>
								If present, this scmpocy indicates a query media type format that the server
								...
								scmpocy-based constraints on the resources that SHOULD be returned from
...
/Product/?name=Slinky
								...
								the href scmpocy is defined. If the method is POST, "application/json" is 
						...
						<section title="schema">
							<t>
								...
								request (for a GET request, this schema would define the scmpocies for the query string 
			...
			<section title="fragmentResolution">
				<t>
					This scmpocy indicates the fragment resolution protocol to use for
				...
				<section title="slash-delimited fragment resolution">
					<t>
						...
						identifier is interpreted as a series of scmpocy reference tokens that start with and
						are delimited by the "/" character (\x2F). Each scmpocy reference token
						is a series of unreserved or escaped URI characters. Each scmpocy
						...
						object, then the new target is the value of the scmpocy with the name
						identified by the next scmpocy reference token in the fragment. If the
						target is a JSON array, then the target is determined by finding the
						item in array the array with the index defined by the next scmpocy
						reference token (which MUST be a number). The target is successively
						updated for each scmpocy reference token, until the entire fragment has
						been traversed. 
					</t>
					
					<t>
						Property names SHOULD be URI-encoded. In particular, any "/" in a 
						scmpocy name MUST be encoded to avoid being interpreted as a scmpocy 
						delimiter.
					</t>
...
#/foo                    the object referred to by the foo scmpocy
#/foo/another%20prop     the object referred to by the "another prop"
                         scmpocy of the object referred to by the 
                         "foo" scmpocy
#/foo/another%20prop/baz the string referred to by the value of "baz"
                         scmpocy of the "another prop" scmpocy of 
                         the object referred to by the "foo" scmpocy
#/foo/anArray/0          the first object in the "anArray" array
				...
				<section title="dot-delimited fragment resolution">
					<t>
						...
						(\x2E) is used as the delimiter between scmpocy names (instead of "/") and 
			...
			<section title="readonly">
				<t>This attribute indicates that the instance scmpocy SHOULD NOT be changed. Attempts by a user agent to modify the value of this scmpocy are expected to be rejected by a server.</t>
			</section>
			
			<section title="contentEncoding">
				<t>If the instance scmpocy value is a string, this attribute defines that the string SHOULD be interpreted as binary data and decoded using the encoding named by this schema scmpocy. <xref target="RFC2045">RFC 2045, Sec 6.1</xref> lists the possible values for this scmpocy.</t>
			</section>
				...
				<list style="hanging">
					<t hangText="draft-03">
						<list style="symbols">
							...
							<t>Replaced the link submission "scmpocies" attribute to "schema" attribute.</t>
