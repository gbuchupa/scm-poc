...
- `options` -- Optional Object, with scmpocies:
...
The object returned will have the following scmpocies:
...
the `#get()` method instead of this scmpocy.
...
Each object has a `name` and a `value` scmpocy. The returned objects may be
safely modified.
